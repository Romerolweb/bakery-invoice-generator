...
describe("PDF Generator", () => {
  ...
  it("should generate PDF successfully", async () => {
    ...
    expect(result.filePath).toContain("test-001.pdf");
  });

  it("should include all required PDF sections", async () => {
    await pdfGenerator.generate(mockReceipt, "test-op-002");
    
    // Verify template methods would be called
    // (This is a simplified test - full template testing would be in integration tests)
    expect(true).toBe(true); // Placeholder for template method verification
  });
});
