...
func handler(ctx context.Context, event events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	switch {
	case req.Method == "POST" && req.Path == "/api/v1/products":
		resp, err = productHandler.HandleCreate(ctx, req)
	case req.Method == "GET" && req.Path == "/api/v1/products":
		resp, err = productHandler.HandleList(ctx, req)
	case req.Method == "GET" && req.PathParams["id"] != "":
		resp, err = productHandler.HandleGet(ctx, req)
	case req.Method == "PUT" && req.PathParams["id"] != "":
		resp, err = productHandler.HandleUpdate(ctx, req)
	case req.Method == "DELETE" && req.PathParams["id"] != "":
		resp, err = productHandler.HandleDelete(ctx, req)
	case req.Method == "GET" && req.Path == "/api/v1/products/search":
		resp, err = productHandler.HandleSearch(ctx, req)
	case req.Method == "GET" && req.Path == "/api/v1/products/categories":
		resp, err = productHandler.HandleListCategories(ctx, req)
	default:
