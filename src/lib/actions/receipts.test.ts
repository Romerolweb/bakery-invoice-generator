...
import { IPdfGenerator, PdfGenerationResult } from '@/lib/services/pdfGeneratorInterface'; // Import interface
...
const mockPdfKitGenerate = vi.fn<[Receipt, string], Promise<PdfGenerationResult>>();
const mockPuppeteerGenerate = vi.fn<[Receipt, string], Promise<PdfGenerationResult>>();
...
describe('Receipt Actions', () => {
    const mockProducts: Product[] = [{ id: 'p1', name: 'Product 1', unit_price: 10, GST_applicable: true, description: 'Desc 1' }];
    const mockSellerProfile: SellerProfile = { name: 'Seller 1', business_address: '1 Seller St', ABN_or_ACN: '123456789', contact_email: 'seller@test.com', phone: '111' };
    const mockCustomer: Customer = { id: 'c1', customer_type: 'business', first_name: 'Cust', last_name: 'Omer', business_name: 'Customer 1 Biz', abn: '987654321', email: 'cust@test.com', phone: '222', address: '1 Cust Rd' };
    ...
    const mockGeneratedReceipt: Receipt = {
        ...
        customer_id: 'c1',
        date_of_purchase: '2024-01-01',
        line_items: [{ product_id: 'p1', quantity: 2, unit_price: 10, line_total: 20, product_name: 'Product 1', description: 'Desc 1', GST_applicable: true }],
  ...
  beforeEach(() => {
    ...
    mockPdfKitGenerate.mockResolvedValue({ success: true, filePath: '/path/to/pdfkit.pdf' });
    mockPuppeteerGenerate.mockResolvedValue({ success: true, filePath: '/path/to/puppeteer.pdf' });
  ...
  describe('createReceipt', () => {
    it('should successfully create a receipt and generate PDF using PDFKit (default)', async () => {
         ...
         expect(createReceiptData).toHaveBeenCalledWith(expect.objectContaining({ customer_id: 'c1' }));
    ...
    it('should fail if a line item product is not found', async () => {
      const inputWithInvalidProduct = { ...mockReceiptInputData, line_items: [{ product_id: 'invalid-p', quantity: 1 }] };
     ...
     it('should return success but indicate PDF failure if generator fails', async () => {
         ...
         mockPdfKitGenerate.mockResolvedValue({ success: false, message: pdfErrorMsg });
     ...
     it('should fail if saving receipt data fails', async () => {
      ...
      it('should calculate totals correctly (including GST)', async () => {
         ...
         const products = [
             { id: 'p1', name: 'Product 1', unit_price: 10, GST_applicable: true, description: '' },
             { id: 'p2', name: 'Product 2', unit_price: 5, GST_applicable: false, description: '' },
         ];
          vi.mocked(getAllProducts).mockResolvedValue(products);

         await createReceipt(inputData);

         expect(createReceiptData).toHaveBeenCalledWith(expect.objectContaining({
             subtotal_excl_GST: 25.00, // 20 + 5
             GST_amount: 2.00, // 10% of 20 (p1)
             total_inc_GST: 27.00, // 25 + 2
         }));
     });
     ...
     it('should calculate totals correctly (excluding GST)', async () => {
          ...
          expect(createReceiptData).toHaveBeenCalledWith(expect.objectContaining({
              ...
              GST_amount: 0.00, // Should be 0 as include_gst is false
              total_inc_GST: 30.00,
          }));
      });
  ...
  describe('getAllReceipts', () => {
    it('should successfully retrieve and sort receipts', async () => {
      const mockUnsortedReceipts: Receipt[] = [
        { receipt_id: 'r2', date_of_purchase: '2023-01-01', customer_id: 'c1', line_items: [], subtotal_excl_GST: 10, GST_amount: 1, total_inc_GST: 11, is_tax_invoice: false, seller_profile_snapshot: mockSellerProfile, customer_snapshot: mockCustomer },
        { receipt_id: 'r1', date_of_purchase: '2023-01-15', customer_id: 'c2', line_items: [], subtotal_excl_GST: 20, GST_amount: 2, total_inc_GST: 22, is_tax_invoice: true, seller_profile_snapshot: mockSellerProfile, customer_snapshot: mockCustomer },
      ];
