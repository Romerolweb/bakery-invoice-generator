...
export class PuppeteerPdfGenerator implements IPdfGenerator {
  ...
  private _initialize(receiptId: string, operationId: string): void {
    ...
    logger.info(
      this._logPrefix,
      `Initializing Puppeteer PDF generation for path: ${this._filePath}`,
    );
  }
  ...
  private async _cleanupFailedPdf(): Promise<void> {
    ...
    await logger.warn(
      funcPrefix,
      `Attempting cleanup for potentially failed Puppeteer PDF: ${this._filePath}`,
    );
    try {
      await fsPromises.access(this._filePath);
      await logger.warn(
        funcPrefix,
        `Deleting incomplete/corrupted PDF: ${this._filePath}`,
      );
      await fsPromises.unlink(this._filePath);
      await logger.info(
        funcPrefix,
        `Deleted incomplete PDF: ${this._filePath}`,
      );
    } catch (error: any) {
  ...
  public async generate(
    ...
    try {
      ...
      await logger.error(
        this._logPrefix,
        "ERROR during Puppeteer PDF generation orchestration",
        error,
      );
      ...
      if (page) {
        try {
          ...
          await logger.debug(
            this._logPrefix,
            "Puppeteer page closed in finally block.",
          );
        } catch (e) {
          await logger.warn(
            this._logPrefix,
