...
// Helper to defer execution to the next event loop tick
...
async function log(
  ...
  // Defer expensive stringification and I/O to make logging non-blocking
  defer(async () => {
    try {
...
Data: [Could not stringify data]`;
      ...
      // Fallback: simple console error if the deferred logging fails
      console.error("[Logger] Error in deferred logging:", error);
    }
  });
}
...
export const logger: {
  ...
  info: (prefix: string, message: string, data?: LogData) =>
    log(LogLevel.INFO, prefix, message, data),
  warn: (prefix: string, message: string, data?: LogData) =>
    log(LogLevel.WARN, prefix, message, data),
  ...
  debug: (prefix: string, message: string, data?: LogData) =>
    log(LogLevel.DEBUG, prefix, message, data),
};
