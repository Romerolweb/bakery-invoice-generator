...
import { promises as fsPromises, createWriteStream, unlinkSync, accessSync } from 'fs'; // Import sync versions too
...
describe('PdfGenerator', () => {
    ...
    // Mock the fs methods we'll use
     ...
     const mockPDFDocumentInstance = {
         ...
         page: { margins: { left: 50, bottom: 50, top: 50, right: 50 }, width: 612, height: 792 }, // Mock page dimensions and margins
    ...
    beforeEach(() => {
        ...
        vi.mocked(path.join).mockImplementation((...args) => {
             ...
             if (args.includes('receipts.json')) { // Example for other paths if needed
                  return path.posix.join(process.cwd(), 'src', 'lib', 'data', 'receipts.json');
             }
    ...
    it('should add header', () => {
         ...
         expect(mockPDFDocumentInstance.text).toHaveBeenCalledWith('TAX INVOICE', { align: 'center' });
    ...
    it('should add seller info', () => {
        const mockSeller: SellerProfile = { name: 'Test Seller', business_address: '123 Test St', ABN_or_ACN: '123', contact_email: 'test@test.com' };
         (pdfGenerator as any)._initialize(mockReceiptId, mockOperationId);
    ...
    it('should add customer info (business)', () => {
        const mockCustomer: Omit<Customer, 'id'> = { customer_type: 'business', first_name: 'Test', last_name: 'Contact', business_name: "Test Biz", abn: '456', email: 'biz@test.com', phone: '123', address: '1 Business Ave' };
         (pdfGenerator as any)._initialize(mockReceiptId, mockOperationId);
         ...
         expect(mockPDFDocumentInstance.text).toHaveBeenCalledWith("Contact: Test Contact");
         // ... check other fields
    });

     it('should add customer info (individual)', () => {
        const mockCustomer: Omit<Customer, 'id'> = { customer_type: 'individual', first_name: 'Indy', last_name: 'Vid', email: 'indy@test.com', phone: '456', address: '2 Person Ln' };
         (pdfGenerator as any)._initialize(mockReceiptId, mockOperationId);
         (pdfGenerator as any)._addCustomerInfo(mockCustomer);
    ...
    it('should add invoice details', () => {
        const mockDate = '2024-01-01T10:00:00.000Z'; // Use ISO string
         ...
         expect(mockPDFDocumentInstance.text).toHaveBeenCalledWith(`Invoice ID: ${mockReceiptId}`);
         ...
         expect(mockPDFDocumentInstance.text).toHaveBeenCalledWith(`Date: 01/01/2024`);
    });

    it('should add line items table (with GST)', () => {
        const mockLineItems: LineItem[] = [{ product_id: 'p1', description: 'Desc 1', quantity: 2, unit_price: 10, line_total: 20, product_name: 'Test Product 1', GST_applicable: true }];
         (pdfGenerator as any)._initialize(mockReceiptId, mockOperationId);
         ...
         expect(mockPDFDocumentInstance.text).toHaveBeenCalledWith('Item', expect.any(Number), expect.any(Number), expect.objectContaining({ underline: true }));
         expect(mockPDFDocumentInstance.text).toHaveBeenCalledWith('GST?', expect.any(Number), expect.any(Number), expect.objectContaining({ underline: true, align: 'center' }));
         // ... check other header columns

         // Check row drawing
         expect(mockPDFDocumentInstance.text).toHaveBeenCalledWith('Test Product 1', expect.any(Number), expect.any(Number), expect.any(Object));
         expect(mockPDFDocumentInstance.text).toHaveBeenCalledWith('Yes', expect.any(Number), expect.any(Number), expect.objectContaining({ align: 'center' }));
         expect(mockPDFDocumentInstance.text).toHaveBeenCalledWith('2', expect.any(Number), expect.any(Number), expect.objectContaining({ align: 'right' }));
         expect(mockPDFDocumentInstance.text).toHaveBeenCalledWith('$10.00', expect.any(Number), expect.any(Number), expect.objectContaining({ align: 'right' }));
         expect(mockPDFDocumentInstance.text).toHaveBeenCalledWith('$20.00', expect.any(Number), expect.any(Number), expect.objectContaining({ align: 'right' }));
    });

     it('should add line items table (without GST)', () => {
        const mockLineItems: LineItem[] = [{ product_id: 'p1', description: 'Desc 1', quantity: 2, unit_price: 10, line_total: 20, product_name: 'Test Product 1', GST_applicable: false }];
         (pdfGenerator as any)._initialize(mockReceiptId, mockOperationId);
         mockPDFDocumentInstance.y = 150; // Simulate starting position
         ...
         expect(mockPDFDocumentInstance.text).not.toHaveBeenCalledWith('GST?', expect.any(Number), expect.any(Number), expect.any(Object));

         // Check row drawing - GST? column should be missing
         expect(mockPDFDocumentInstance.text).not.toHaveBeenCalledWith('Yes', expect.any(Number), expect.any(Number), expect.any(Object));
         expect(mockPDFDocumentInstance.text).not.toHaveBeenCalledWith('No', expect.any(Number), expect.any(Number), expect.any(Object));
    });
    ...
    it('should add totals (with GST)', () => {
         ...
         expect(mockPDFDocumentInstance.text).toHaveBeenCalledWith(`Subtotal (ex GST):`, expect.any(Number), expect.any(Number), expect.any(Object));
         expect(mockPDFDocumentInstance.text).toHaveBeenCalledWith(`$100.00`, expect.any(Number), expect.any(Number), expect.any(Object));

         expect(mockPDFDocumentInstance.text).toHaveBeenCalledWith(`GST Amount (10%):`, expect.any(Number), expect.any(Number), expect.any(Object));
         expect(mockPDFDocumentInstance.text).toHaveBeenCalledWith(`$10.00`, expect.any(Number), expect.any(Number), expect.any(Object));

         expect(mockPDFDocumentInstance.text).toHaveBeenCalledWith(`Total Amount:`, expect.any(Number), expect.any(Number), expect.any(Object));
         expect(mockPDFDocumentInstance.text).toHaveBeenCalledWith(`$110.00`, expect.any(Number), expect.any(Number), expect.any(Object));
    });
     ...
     it('should add totals (without GST)', () => {
         ...
         expect(mockPDFDocumentInstance.text).not.toHaveBeenCalledWith(`GST Amount (10%):`, expect.any(Number), expect.any(Number), expect.any(Object));
    ...
    it('should clean up failed PDF', async () => {
     ...
     it('should not attempt unlink if file does not exist during cleanup', async () => {
        ...
        (pdfGenerator as any)._logPrefix = '[test]';
         ...
         expect(logger.info).toHaveBeenCalledWith(expect.stringContaining(mockFilePath), expect.stringContaining('did not exist'));
    });
    ...
    it('should generate PDF successfully', async () => {
       const mockLineItems: LineItem[] = [{ product_id:'p1', description: 'Desc', quantity: 1, unit_price: 10, line_total: 10, product_name:'Test Product', GST_applicable: false }];
        ...
        const mockReceipt: Receipt = {
            receipt_id: mockReceiptId, customer_id: 'test-customer', date_of_purchase: '2024-01-01T00:00:00.000Z', line_items: mockLineItems, subtotal_excl_GST: 100, GST_amount: 10, total_inc_GST: 110, is_tax_invoice: true, seller_profile_snapshot: mockSeller, customer_snapshot: mockCustomer as Customer // Cast needed here
        };
         ...
         expect(mockPDFDocumentInstance.text).toHaveBeenCalledWith('TAX INVOICE', expect.any(Object));
         ...
         expect(mockPDFDocumentInstance.text).toHaveBeenCalledWith('Test Product', expect.any(Number), expect.any(Number), expect.any(Object));
         expect(mockPDFDocumentInstance.text).toHaveBeenCalledWith('$110.00', expect.any(Number), expect.any(Number), expect.any(Object));
    });


     it('should handle error during generation and cleanup', async () => {
        const mockLineItems: LineItem[] = [{ product_id:'p1', description:'Desc', quantity: 1, unit_price: 10, line_total: 10, product_name:'Prod', GST_applicable: false }];
        const mockCustomer: Omit<Customer, 'id'> = { customer_type: 'individual', first_name: 'T', last_name: 'C', email:'e@m' };
        const mockSeller: SellerProfile = { name: 'S', business_address:'A', ABN_or_ACN:'1', contact_email:'e@m'};
        const mockReceipt: Receipt = {
            receipt_id: mockReceiptId, customer_id: 'cust1', date_of_purchase: '2024-01-01T00:00:00.000Z', line_items: mockLineItems, subtotal_excl_GST: 100, GST_amount: 10, total_inc_GST: 110, is_tax_invoice: true, seller_profile_snapshot: mockSeller, customer_snapshot: mockCustomer as Customer // Cast needed
        };
         ...
         // Simulate an error during PDF content addition (e.g., in _addTotals)
          ...
          mockPDFDocumentInstance.text.mockImplementation((text: string) => {
              if (text.startsWith('$110.00')) { // Simulate error when adding total
                  throw testError;
              }
         ...
         expect(result.message).toContain('Failed to generate PDF: Simulated content error');
         expect(mockedUnlinkSync).toHaveBeenCalledWith(expect.stringContaining(`${mockReceiptId}.pdf`)); // Check cleanup happened
     ...
     it('should handle initialization error', async () => {
         ...
         MockPDFDocument.mockImplementation(() => { throw initError; });
         const mockReceipt: Receipt = { /* minimal mock receipt */ } as any;
