...
async function readReceiptsFile(): Promise<Receipt[]> {
  ...
  try {
    ...
    await logger.debug(funcPrefix, `Ensured data directory exists: ${dataDirectory}`);

    const data = await fs.readFile(receiptsFilePath, 'utf8');
    await logger.debug(funcPrefix, `Successfully read receipts file: ${receiptsFilePath}`);
    return JSON.parse(data) as Receipt[];
  } catch (error: any) {
    if (error.code === 'ENOENT') {
        await logger.warn(funcPrefix, `Receipts file not found at ${receiptsFilePath}, returning empty array.`);
        return []; // File doesn't exist, return empty array
    }
    await logger.error(funcPrefix, `Error reading receipts file: ${receiptsFilePath}`, error);
...
async function writeReceiptsFile(receipts: Receipt[]): Promise<void> {
  ...
  try {
    ...
    await logger.debug(funcPrefix, `Successfully wrote receipts file: ${receiptsFilePath}`);
  } catch (error: any) {
     await logger.error(funcPrefix, `Error writing receipts file: ${receiptsFilePath}`, error);
...
export async function createReceipt(newReceipt: Receipt): Promise<Receipt | null> {
  ...
  await logger.debug(funcPrefix, `Attempting to create receipt with ID: ${newReceipt.receipt_id}`);
  try {
    const receipts = await readReceiptsFile();
    // Optional: Check if receipt ID already exists to prevent duplicates
    if (receipts.some(r => r.receipt_id === newReceipt.receipt_id)) {
        await logger.warn(funcPrefix, `Receipt with ID ${newReceipt.receipt_id} already exists. Creation aborted.`);
    ...
    await logger.info(funcPrefix, `Receipt created successfully: ${newReceipt.receipt_id}`);
    return newReceipt;
  } catch (error) {
...
export async function getReceiptPdfPath(receiptId: string): Promise<string | null> {
    ...
    try {
        ...
        if (error.code === 'ENOENT') {
             ...
             await logger.info(funcPrefix, `PDF file not found at ${filePath}. It might be generating or failed.`);
        } else {
            // Log other errors (e.g., permission issues) but still return null
            await logger.error(funcPrefix, `Error accessing PDF file at ${filePath}`, error);
        }
...
export async function getReceiptPdfContent(receiptId: string): Promise<Buffer | null> {
    ...
    const filePath = path.join(pdfDirectory, `${receiptId}.pdf`);
    await logger.debug(funcPrefix, `Attempting to read PDF content from: ${filePath}`);
    try {
        ...
        await logger.info(funcPrefix, `Successfully read PDF content (${pdfBuffer.length} bytes).`);
        return pdfBuffer;
    } catch (error: any) {
        // Catch potential errors during readFile itself (though access check reduces likelihood)
        await logger.error(funcPrefix, `Error reading PDF file content at ${filePath}`, error);
        return null;
    }
}
