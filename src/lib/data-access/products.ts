...
async function readProductsFile(): Promise<Product[]> {
  ...
  try {
    ...
    await logger.debug(funcPrefix, `Successfully read products file: ${productsFilePath}`);
    return JSON.parse(data) as Product[];
  } catch (error: any) {
    if (error.code === 'ENOENT') {
       await logger.warn(funcPrefix, `Products file not found at ${productsFilePath}, returning empty array.`);
       return []; // File doesn't exist, return empty array
    }
    await logger.error(funcPrefix, `Error reading products file: ${productsFilePath}`, error);
...
async function writeProductsFile(products: Product[]): Promise<void> {
  ...
  try {
    ...
    await logger.debug(funcPrefix, `Successfully wrote products file: ${productsFilePath}`);
  } catch (error: any) {
     await logger.error(funcPrefix, `Error writing products file: ${productsFilePath}`, error);
...
export async function saveAllProducts(products: Product[]): Promise<boolean> {
   ...
   await logger.debug(funcPrefix, `Attempting to save ${products.length} products.`);
  try {
...
export async function createProduct(product: Product): Promise<Product | null> {
   ...
   await logger.debug(funcPrefix, `Attempting to create product with ID: ${product.id}`);
  try {
    const products = await readProductsFile();
    // Check if product with the same ID already exists
    if (products.some(p => p.id === product.id)) {
        await logger.warn(funcPrefix, `Product with ID ${product.id} already exists. Creation aborted.`);
        return null; // Or throw an error if preferred
    }
    products.push(product);
    await writeProductsFile(products);
    await logger.info(funcPrefix, `Product created successfully: ${product.id}`);
    return product;
  } catch (error) {
