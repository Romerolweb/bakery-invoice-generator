...
async function readSellerProfileFile(): Promise<SellerProfile | null> {
  ...
  try {
    ...
    await logger.debug(funcPrefix, `Successfully read seller profile file: ${sellerProfileFilePath}`);
    return JSON.parse(data) as SellerProfile;
  } catch (error: any) {
    if (error.code === 'ENOENT') {
       await logger.warn(funcPrefix, `Seller profile file not found at ${sellerProfileFilePath}, returning null.`);
       return null; // File doesn't exist
    }
    await logger.error(funcPrefix, `Error reading seller profile file: ${sellerProfileFilePath}`, error);
...
async function writeSellerProfileFile(profile: SellerProfile): Promise<void> {
  ...
  try {
    ...
    await logger.debug(funcPrefix, `Successfully wrote seller profile file: ${sellerProfileFilePath}`);
  } catch (error: any) {
    await logger.error(funcPrefix, `Error writing seller profile file: ${sellerProfileFilePath}`, error);
