...
async function readCustomersFile(): Promise<Customer[]> {
  ...
  try {
    ...
    await logger.debug(funcPrefix, `Successfully read customers file: ${customersFilePath}`);
    return JSON.parse(data) as Customer[];
  } catch (error: any) {
    if (error.code === 'ENOENT') {
      await logger.warn(funcPrefix, `Customers file not found at ${customersFilePath}, returning empty array.`);
      return []; // File doesn't exist, return empty array
    }
    await logger.error(funcPrefix, `Error reading customers file: ${customersFilePath}`, error);
...
async function writeCustomersFile(customers: Customer[]): Promise<void> {
  ...
  try {
    ...
    await logger.debug(funcPrefix, `Successfully wrote customers file: ${customersFilePath}`);
  } catch (error: any) {
     await logger.error(funcPrefix, `Error writing customers file: ${customersFilePath}`, error);
...
export async function createCustomer(customerData: Omit<Customer, 'id'>): Promise<Customer | null> {
  ...
  try {
    ...
    await logger.info(funcPrefix, `Customer created successfully with ID: ${newCustomer.id}`);
    return newCustomer;
  } catch (error) {
