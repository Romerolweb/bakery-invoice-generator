...
import { getProducts, addProduct, updateProduct, deleteProduct } from '@/lib/actions/products';
...
import { Loader2, PlusCircle, Edit, Trash2, CircleDollarSign } from 'lucide-react';

const productSchema = z.object({
  id: z.string().optional(), // Present when editing
  name: z.string().min(1, 'Product name is required'),
  description: z.string().optional(),
  unit_price: z.coerce.number({invalid_type_error: "Price must be a number"}).min(0, 'Price cannot be negative'),
...
export default function ProductsPage() {
  ...
  const handleAddNewProduct = () => {
    ...
    form.reset({ name: '', description: '', unit_price: 0, GST_applicable: false });
    ...
    try {
      ...
      if (result.success && result.product) {
        ...
        await fetchProducts(); // Refetch list
         ...
         toast({
          title: "Error",
          description: result.message || `Failed to ${editingProduct ? 'update' : 'add'} product.`,
          variant: "destructive",
        });
      }
    } catch (error) {
               ...
               />
                <FormField
                  ...
                  <FormItem>
                      ...
                      <FormControl>
                      <Textarea placeholder="Flaky, buttery pastry" {...field} />
                      </FormControl>
                      <FormMessage />
                  </FormItem>
                  )}
                  ...
                  <FormItem>
                      ...
                      <FormControl>
                       <div className="relative">
                           ...
                           <Input type="number" step="0.01" min="0" placeholder="3.50" className="pl-8" {...field} />
                       </div>
                      </FormControl>
                      <FormMessage />
                  </FormItem>
      ...
      <Card>
        ...
        <CardContent>
             {isLoading ? (
                 ...
                 <p className="text-center text-muted-foreground py-4">No products found. Add one to get started!</p>
            ): (
            <Table>
                <TableHeader>
                <TableRow>
                ...
                {products.map((product) => (
                    ...
                    <TableCell>{product.GST_applicable ? 'Yes' : 'No'}</TableCell>
